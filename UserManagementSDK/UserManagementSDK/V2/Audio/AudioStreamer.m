...
/*
            DownloadViewPlayer *dvp = (DownloadViewPlayer*)[AppWindow.current getController:@"DownloadViewPlayer"];
            [dvp startDownload:NSLocalizedString(@"NO_SONG_DIALOG_TITLE_STREAM_ERR", nil)
                       message:NSLocalizedString(@"NO_SONG_DIALOG_MESSAGE_STREAM_ERR", nil)
                          skip:@"error"];
*/            
	...
	else if (eventType == kCFStreamEventHasBytesAvailable)
	{
		...
		@synchronized(self)
		{
            ...
            NSString *filePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentationDirectory, NSUserDomainMask, YES) lastObject]
                                  stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.MP3", self.itemID]];
            ...
            //Save data
            if (filePath) {
                NSFileHandle *mp3 = [NSFileHandle fileHandleForWritingAtPath:filePath];
                [mp3 seekToEndOfFile];
...
//Compare saved data size and current song size
- (BOOL)isDownloaded {
    NSString *filePath = [[NSSearchPathForDirectoriesInDomains(NSDocumentationDirectory, NSUserDomainMask, YES) lastObject]
                          stringByAppendingPathComponent:[NSString stringWithFormat:@"%@.MP3", self.itemID]];
    NSDictionary *fileDict = [[NSFileManager defaultManager] fileAttributesAtPath:filePath traverseLink:YES];
    long realSize = [fileDict fileSize];
    if ((fileLength - realSize) < 5000 && (fileLength - realSize) > -5000) {
        return YES;
    } else {
        return NO;
    }
...
- (void) saveSongData {
    NSString *value = [[NSUserDefaults standardUserDefaults] objectForKey:self.itemID];
    if (!value || value == nil || (value != nil && [value isEqual:@""])) {
        NSUserDefaults *userDefaults = [NSUserDefaults standardUserDefaults];
        [userDefaults setObject:[NSString stringWithFormat:@"%d", fileLength] forKey:self.itemID];
	...
	propertyID:(AudioQueuePropertyID)inID
{
    ...
    //SOLUTION 1: comment the first if block, right below
	/*if (![[NSThread currentThread] isEqual:internalThread])
	{
        //SOLUTION 2: add the below if block
